...
const app = express();
app.get('/', (req, res) => {	res.send(`Subscribe to MaxDev6966`);
});
app.listen(3000, () => {
...
if (attachm) {
  ...
  .setStyle('grey') //default: blurple
  .setEmoji('939813605254238208') 
  .setLabel(`${i}`) //default: NO_LABEL_PROVIDED
  .setID(back)
  let button2 = new disbut.MessageButton()
  .setStyle('grey') //default: blurple
  .setEmoji('939800610088312863')
...
.setColor(`RANDOM`).setThumbnail(message.author.avatarURL({ dynamic: true }))
.setDescription(`Thanks for Suggesting your Idea <#${message.channel.id}>`)
.setTimestamp()
.setFooter('Powered by MAX GAMING YT')
  
message.author.send(dm).catch(console.error) 
  }
}); 
